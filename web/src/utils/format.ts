...
export const getMonthIndexFromOne = (value: string) => {
  const indexOfValue = monthNames.indexOf(value);
  if (indexOfValue !== -1) {
    const str = (indexOfValue + 1).toString();
    const pad = "00";
    const ans = pad.substring(str.length) + str;
    return ans;
  }
  return "";
};
...
export const dateFormat = (date: Date) => {
  ...
  } ${date.getFullYear()}, ${stringHours}:${stringMinutes} ${ampm}`;
};
